...
def open_epub(in_file):
    theEpub = ZP(in_file)
    return theEpub

def get_epub_content_soup(_ePub):
    try:
...
def get_epub_general_info(content_soup):
    meta_info = {}
...
def get_epub_content_lists(content_soup):
    spine_list = []
    img_list = []
    text_list = []
    css_list = []
...
def get_epub_section(_ePub, section):
    try:
...
def preprocess_image(_ePub, image):
    try:
